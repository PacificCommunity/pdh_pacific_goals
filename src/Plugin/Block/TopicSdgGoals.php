 ...
 * @Block(
 *   id = "topic_sdg_goals",
 *   admin_label = @Translation("Topic SDG Goals")
 * )
 */
class TopicSdgGoals extends BlockBase {
  ...
  public function build() {
    ...
    $sdgs = [];
    if (!empty($config['topic_sdg_goals'])) {
      $sdgs = $config['topic_sdg_goals'];
    }
    
    $terms =\Drupal::entityTypeManager()->getStorage('taxonomy_term')->loadTree('sdg_goals', 0, 1, true);
    ...
    foreach ($terms as $term) {
      ...
      $items[] = [
        ...
        '#template' => !empty($config['topic_sdg_goals'][$w+1])?$target_on:$target_off,
        '#context' => [
          'weight' => $w+1,
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $terms =\Drupal::entityTypeManager()->getStorage('taxonomy_term')->loadTree('sdg_goals');
    ...
    $form['topic_sdg_goals'] = [
      '#type' => 'checkboxes',
      '#title' => $this->t('Related SDGs goals'),
      '#options' => $options,
      // '#description' => $this->t('Yeah I dont need this'),
      '#default_value' => isset($config['topic_sdg_goals']) ? $config['topic_sdg_goals'] : '',
    ];
  ...
  public function blockSubmit($form, FormStateInterface $form_state) {
    ...
    // _dbg($values);
    $this->configuration['topic_sdg_goals'] = $values['topic_sdg_goals'];
